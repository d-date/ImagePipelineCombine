...
class FetcherTests: XCTestCase {
  ...
  func testFetchPNG() {
    ...
    let url = URL(string: "https://satyr.io/200x150?type=png")!
    let fetcher = Fetcher()
    fetcher.fetch(url)
      .sink { (completion) in
        switch completion {
  ...
  func testFetchJPEG() {
    ...
    let url = URL(string: "https://satyr.io/200x150?type=jpg")!
    let fetcher = Fetcher()
    fetcher.fetch(url)
      .sink { (completion) in
        switch completion {
  ...
  func testFetchGIF() {
    ...
    let url = URL(string: "https://satyr.io/200x150?type=gif")!
    let fetcher = Fetcher()
    fetcher.fetch(url)
      .sink { (completion) in
        switch completion {
  ...
  func testFetchWebP() {
    ...
    let url = URL(string: "https://satyr.io/200x150?type=webp")!
    let fetcher = Fetcher()
    fetcher.fetch(url)
      .sink { (completion) in
        switch completion {
